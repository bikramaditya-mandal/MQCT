...
!! Bikram Start Dec 2020:
	  ...
	  if(myid.eq.0) call bk_remove_matrix_dir
      CALL MPI_BARRIER( MPI_COMM_WORLD, ierr_mpi )	 
	  
	  if(total_size.gt.nproc) then
...
!!!!  COMPUTING MATRIX	 
     ...
     & PRINT*, "COMPUTING MATRIX ELEMENTS STARTED"	  
	  ...
	  mij_k_skip = .false.
      IF(mij_k_skip) CYCLE !!! SKIP SOME ELEMENTS IF NESSEACRY		  
      DO i=1,n_r_coll
	  IF(mij_k_skip) CYCLE
...
!	  bgn_tym = MPI_Wtime()											!Bikram
	  call bk_print_matrix_formatted_printing (k_st_mpi, k_fn_mpi, k,
     & bk_mat_counter, bk_mat_temp1, mt_chk)
	  bk_mat_counter = 0
!	  end_tym = MPI_Wtime()											!Bikram
	  endif
!	  calc_tym = end_tym - bgn_tym									!Bikram
...
!	  calc_tym = end_tym - bgn_tym									!Bikram
!	  tot_tym_calc = tot_tym_calc + calc_tym						!Bikram
      bk_mat_temp(i) = intgeral*conv_unit_e
      IF(ABS(bk_mat_temp(1)).LT.MIJ_ZERO_CUT) THEN
      bk_mat_temp(:) = 0d0
      mij_k_skip = .TRUE.	  
      ENDIF
      ENDIF	  
      ENDDO
	  bk_mat_temp1(:,1) = bk_mat_temp(:)
!	  bgn_tym = MPI_Wtime()											!Bikram
	  call bk_print_matrix_formatted_printing (k,k,k,1,bk_mat_temp1,1)
!	  end_tym = MPI_Wtime()											!Bikram
!	  calc_tym = end_tym - bgn_tym									!Bikram
!	  tot_tym_prn = tot_tym_prn + calc_tym							!Bikram
      CALL MPI_BARRIER( MPI_COMM_WORLD, ierr_mpi )	 
...
!	  print *, 'calc_bk', myid, tot_tym_calc						!Bikram
      CALL MPI_BARRIER( MPI_COMM_WORLD, ierr_mpi )	 
      CALL MPI_BARRIER( MPI_COMM_WORLD, ierr_mpi )	 
...
!	  print *, myid, 'checking about program run'
!	  if(mpi_task_per_traject.ne.1) then
!	  allocate(Mat_el(n_r_coll,total_size))
!	  allocate(Mat_el_der(n_r_coll,total_size))
!	  allocate(K_SKIPPED_BY_ROUTINE(total_size))
	  
!	  if(myid.eq.0) then
!	  do k = 1, total_size
!	  call bk_print_matrix_formatted_reading(k,bk_mat_temp)
...
!	  do k = 1, total_size_mpi
	  bgn_tym = MPI_Wtime()											!Bikram
	  call bk_print_matrix_formatted_reading
     & (total_size_mpi, portion_of_MIJ_per_task(1,myid+1), 
     & portion_of_MIJ_per_task(2,myid+1), Mat_el)
!	  end_tym = MPI_Wtime()											!Bikram
!	  calc_tym = end_tym - bgn_tym									!Bikram
...
!	  Mat_el(:,k) = bk_mat_temp(:)
      ...
      ENDIF
!	  end do
!	  print *, 'rd_bk', myid, tot_tym_rd							!Bikram
...
!	  tot_tym_prn = tot_tym_prn + calc_tym							!Bikram
     ...
     & ,"""",MATRIX_NAME_MIJ_UF,""""	  
	  ...
	  call bk_print_matrix_info_formatted
	  endif
...
!	  print *, i, myid, nproc
!	  call bk_print_matrix_formatted_printing (i)
!	  print *, 'finished', i
!	  end do
!	  endif
...
!	  if (print_matrix_defined .and. bikram_mij_multiprint) then
	  ...
	  if (myid.eq.0) then
      call bk_read_matrix_info_formatted
      end if
      CALL MPI_BCAST(R_COM, n_r_coll, MPI_REAL8,0,
     &  MPI_COMM_WORLD,ierr_mpi)
...
!!!! COMPUTING THE REST OF THE MATRIX MIJ
      ...
      IF(myid.lt.bk_mij_addition) THEN
      k = myid + 1 + total_size_old
      CALL	INTEGRATOR_TEMP(intgeral,k,i)
      bk_mat_temp(i) = intgeral*conv_unit_e
      IF(ABS(bk_mat_temp(1)).LT.MIJ_ZERO_CUT) THEN
      bk_mat_temp(:) = 0d0
...
!      CLOSE(333)	
     ...
     & ind_mat(1,k),ind_mat(2,k)
	  ...
	  SUBROUTINE bk_remove_matrix_dir
! This subroutine is created by Bikramaditya Mandal, Nov 2020
	  ...
	  SUBROUTINE bk_print_matrix_info_formatted
! This subroutine is created by Bikramaditya Mandal, Nov 2020
      USE CONSTANTS	  
      USE VARIABLES
      USE MPI	  
     ...
     & ,j2_ch(i),ka2_ch(i),kc2_ch(i),parity_state(st)
	  ...
	  SUBROUTINE bk_print_matrix_formatted_printing(bk_n1, bk_n2, 
     & bk_nn, bk_ncount, bk_mat_array, mat_chk)
! This subroutine is created by Bikramaditya Mandal, Nov 2020
      USE CONSTANTS	  
      USE VARIABLES
      USE MPI	  
     ...
     & bk_mat_array(ii,kk)
	  ...
	  SUBROUTINE bk_print_matrix_formatted_reading(bk_nn, bk_st,
     & bk_fn, bk_mat_array)
! This subroutine is created by Bikramaditya Mandal, Nov 2020
      USE CONSTANTS	  
      USE VARIABLES
...
! This subroutine is created by Bikramaditya Mandal, Nov 2020
	  ...
	  write(bk_dir_temp_3, '(a,a)') 
     & bk_dir, '/MATRIX_COMBINE.sh'
...
! This subroutine is created by Bikramaditya Mandal, Nov 2020
	  ...
	  write(bk_dir_temp_1, '(a,a)') 
     & bk_dir, '/MATRIX.DAT'
	  bk_dir_temp_2 = trim(bk_dir_temp_1)
	  INQUIRE(FILE=bk_dir_temp_2, EXIST=file_exst )
      IF(.not.file_exst) THEN 
      PRINT*, "ERROR: COMBINED MATRIX FILE NOT FOUND"
	  return
	  stop
	  call MPI_FINALIZE(ierr_mpi)
	  end if	  
	  ...
	  write(bk_dir_temp_3, '(a,a)') 
     & bk_dir, '/MATRIX_SPLIT.sh'
	  bk_dir_temp_4 = trim(bk_dir_temp_3)
	  open(11,file = bk_dir_temp_4)
	  write(11,*)'#!/bin/sh'
	  write(bk_dir_temp_1, '(a,i0)') 
     & 'ncount=',mpi_task_per_traject
	  write(11,*) trim(bk_dir_temp_1)
	  write(bk_dir_temp_1, '(a)') 
     & 'n[0]=0'
	  write(11,*) trim(bk_dir_temp_1)
	  
	  do i = 1, mpi_task_per_traject
	  write(bk_dir_temp_1, '(a,i0,a,i0,a)') 
     & 'n[',i,']=',portion_of_MIJ_per_task(2,i)*n_r_coll
	  bk_dir_temp_2 = trim(bk_dir_temp_1)
	  write(11,*) bk_dir_temp_2
	  end do
	  ...
	  do i = 1, mpi_task_per_traject
	  write(bk_dir_temp_1, '(a,i0,a,i0,a)') 
     & 'nn[',i,']=',portion_of_MIJ_per_task(2,i)
	  bk_dir_temp_2 = trim(bk_dir_temp_1)
	  write(11,*) bk_dir_temp_2
	  end do
	  
	  write(11,*) 'i=0'
	  write(11,*) 'while [ "$i" -lt $ncount ]'
	  write(11,*) 'do'
	  write(11,*) 'stt1="${n[$i]}"'
	  write(11,*) 'st1=`expr $stt1 + 1`'
	  write(11,*) 'stt2=`expr $i + 1`'
	  write(11,*) 'st2="${n[$stt2]}"'
	  write(11,*)
	  write(11,*) 'stt11="${nn[$i]}"'
	  write(11,*) 'st11=`expr $stt11 + 1`'
	  write(11,*) 'stt22=`expr $i + 1`'
	  write(11,*) 'st22="${nn[$stt22]}"'
	  write(11,*) 'name="MTRX_"$st11"_"$st22".DAT"'
	  write(11,*)
	  write(11,*) 'sed -n "$st1,$st2 p" MATRIX.DAT >> $name'
	  write(11,*)
	  write(11,*) 'i=`expr $i + 1`'
	  write(11,*)
	  write(11,*) 'done'
	  ...
	  call getcwd(bk_dir_temp_5)
	  call chdir(trim(bk_dir))
	  call system ( "chmod +x MATRIX_SPLIT.sh" )
	  call system ( "./MATRIX_SPLIT.sh" )
	  print *, 'need work to do on split'
	  call chdir(trim(bk_dir_temp_5))
