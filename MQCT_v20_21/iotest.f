...
! Bikram End. 
     ...
     & r_grid_vib2(:)	 
	  ...
	  character(len = *),parameter :: bk_directory = 'APPROX_TRAJS'			!Bikram April 2020
...
!      PRINT*,"buffer = ",buffer	 
     ...
     & key,place,decrement,i
      ...
      CHARACTER(LEN=20) :: user_k_word="USER_DEFINED_LEVELS="   !CASE(2)
      ...
      CHARACTER(LEN=10)	:: bk_adia = "ADIABATIC="                !CASE(75)				!Bikram April 2020
      CHARACTER(LEN=10)	:: bk_save_traj = "SAVE_TRAJ="                !CASE(76)				!Bikram April 2020
      CHARACTER(LEN=1) buffer
      LOGICAL key_used
...
c      PRINT*,n_r_vib,grid_defined	!!!!!!!!!! DELETE  
     ...
     &  CALL ERROR_SIGNALING(29,3)	
      ...
      INTEGER, PARAMETER :: num_key_word = 37	  
      INTEGER length,posit,key_word_used(num_key_word),
     & key,place,decrement,i
      CHARACTER(LEN=length) inp
      ...
      CHARACTER(LEN=7)	:: bk_mij_multiprint = "BK_MIJ="               !CASE(37) 	   		!Bikram Nov 2020   	  
      CHARACTER(LEN=1) buffer
      LOGICAL key_used
      IF(length.le.0) RETURN
      key_used = .FALSE.	  
...
c      PRINT*,"number_of_channels = ",number_of_channels     
     ...
     & STATUS="OLD",ACTION="READ")
      ...
      jmax_included = 0	  
      DO i=1,number_of_channels
      READ(325,'(i5,1x,i4,1x,i4,1x,i4,1x,e17.10)',
     & IOSTAT = stat_of_file)
...
!      READ(456,*)wf_vib_part_diatom2(i,k) ! VALUE OF WAVEFUNCTION FOR k-channel at i-point of the grid
      ...
      ELSE
      IF(morse_pot_defined) THEN
      IF(morse12_re(1).le.0 .or. morse12_re(2).le.0 )
     & STOP "ERROR: YOU MUST SUPPLY R_EQ FOR MORSE POTENTIAL"
      IF(atomic_masses(1)*atomic_masses(2).le.0) STOP
     & "ERROR: SUPPLY MASS OF ATOMS"
     ...
     & STOP"ERROR: CHECK INPUT-DE1 SHOULD MUCH LESS THAN BE1"	
      ...
      IF(XE1.le.0 .or.XE1.ge.1 ) STOP "ERROR:SUPPLY POSITVE XE1<1"
	  
      IF(BE2.le.0) STOP "ERROR:SUPPLY POSITVE BE2"
      IF(DE2.GT.BE2)
     & STOP"ERROR: CHECK INPUT-DE2 SHOULD MUCH LESS THAN BE2"	
      IF(WE2.le.0 ) STOP "ERROR:SUPPLY POSITVE WE2"
      IF(XE2.le.0 .or.XE1.ge.1 ) STOP "ERROR:SUPPLY POSITVE XE2<1"
      IF(MYID.eq.0) THEN	  
...
c      PRINT*,"number_of_channels = ",number_of_channels     
 ...
 !! PRINTINMG
     ...
     & STATUS="OLD",ACTION="READ")
      ...
      jmax_included = 0	  
      DO i=1,number_of_channels
      READ(325,'(i5,1x,i3,1x,i3,1x,i3,1x,i3,e17.10)',
     & IOSTAT = stat_of_file)
...
!	  PRINT*,"ERROR:NUMBER OF CHANNELS",number_of_channels
 ...
 !! PRINTINMG
     ...
     & STATUS="OLD",ACTION="READ")
      ...
      DO i=1,number_of_channels
      READ(325,
     & '(i5,1x,i3,1x,i3,1x,i3,1x,i3,1x,i3,1x,i3,e17.10)',
     & IOSTAT = stat_of_file)
...
c      PRINT*,"e_temp,="	, e_temp(i) 
     ...
     & "ERROR: J2 MUST BE NO LESS THAN K2. CHECK THE INPUT CHANNELS."
      IF(k2_ch(i).eq.0 .and. eps2_ch(i).ne.1)
     &	  STOP"ERROR:K2=0, SO PARITY MUST BE 1"
