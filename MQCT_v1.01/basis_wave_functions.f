...
! This subroutine is written by Alexander Semenov
     ...
     & coeff = M1_VECTORS(chnn_nmb,1:2*J+1)
      IF(A.eq.A_I .and. B.eq.B_I .and. C.eq.C_I)
     ...
     & coeff = M2_VECTORS(chnn_nmb,1:2*J+1)
      ...
      wfi = wfi + dwi*coeff(i)	  
      ENDDO
      IF(wfr.eq.0d0 .and. wfi.eq.0d0) STOP "ERROR:ASYM TOP MATRIX"	  
...
! This subroutine is written by Alexander Semenov
      ...
      IF(P.ne.0) STOP "ERROR: WRONG LEVEL DEFINITION FOR SYMMETRIC TOP"	  
      ELSE	  
...
! This subroutine is written by Alexander Semenov
     ...
     &	  STOP"ERROR:TRINAGULAR RULE"
      ...
      wfr =wfr +  clb_grd*(wfr1*wfr2 - 
     &	wfi1*wfi2)
      wfi =wfi +  clb_grd*(wfr1*wfi2 + 
     &	wfi1*wfr2)	 
      ENDIF	 
      ENDDO  	 
